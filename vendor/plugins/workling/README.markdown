...
## Using a Synchronous AMQP Client

While the Async AMQP client is fast and non-blocking, it can't be used in non-evented Mongrel or Phusion Passenger. The SyncAmqpClient is designed to use either the Bunny or Carrot gems to provide AMQP support for non-evented servers, as well as adding support for return message support.
...
In addition to the :amqp_options in workling.yml used by the Async AMQP client, the following strucure is required for this client:
...
Note that you can select message persistence and queue durability on a per-direction basis. It's important to know that only if you have both set as true will a message survive a RabbitMQ server crash.

# Using Amazon SQS

Amazon SQS is a queue system that is hosted by Amazon as part of the AWS suite of tools. It's useful if you have systems deployed to Amazon, or if you just don't want the hassle of hosting your own queue and don't mind some (significant) latency.
...
It's important to note that due to the behaviour of Amazon SQS, it's not allowed to have a ':' character in the name of a queue. As a result, if you have defined the aws: key in your workling.yml, the queue names will use a '-' instead. This means that if you are currently running a queue in production and you define this key, you will no longer be able to access that queue as Workling will not look for it any more.

This will not be a problem for most people, but forewarned is forearmed!
