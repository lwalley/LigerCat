...
describe "A BlastQuery", :shared => true do
...
describe "An invalid BlastQuery", :shared => true do
  before(:each) do
    BlastWorker.stub!(:async_execute_search)   
    @query = BlastQuery.new
  end
  ...
  it "should not launch a BlastWorker" do
    BlastWorker.should_not_receive(:async_execute_search)
    @query.save
  end
end 

describe "A valid BlastQuery" do
  it_should_behave_like "A BlastQuery"
  
  before(:each) do
    @query = BlastQuery.new(:fasta_data => '>valid_data\natcgatcg')
    @query.stub!(:id).and_return(123)
  end
  ...
  it "should launch a BlastWorker after creation" do
...
describe "BlastQuery#perform_query!" do
  ...
  it "should create a LigerEngine with a Genbank search and a composite processor and let it run" do
    LigerEngine::Engine.should_receive(:new).with(an_instance_of(LigerEngine::SearchStrategies::GenbankSearchStrategy),
                                                  an_instance_of(LigerEngine::ProcessingStrategies::TagCloudAndHistogramProcessor) ).and_return(@mocked_liger_engine = mock("LigerEngine"))
